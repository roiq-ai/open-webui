...
async def get_session_user_chat_list(
    ...
    chat = await Chats.get_chat_list_by_user_id(user.id, skip, limit)
    return chat
...
async def get_shared_chat_by_id(share_id: str, user=Depends(get_verified_user)):
    ...
    if user.role == "user":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGES.NOT_FOUND
        )
    if chat:
        return ChatResponse(**{**chat.model_dump(), "chat": json.loads(chat.chat)})
    else:
...
async def get_user_chat_list_by_tag_name(
    ...
    chat_ids = [
        chat_id_tag.chat_id
        for chat_id_tag in await Tags.get_chat_ids_by_tag_name_and_user_id(
            form_data.name, user.id
        )
    ]

    chats = await Chats.get_chat_list_by_chat_ids(
        chat_ids, form_data.skip, form_data.limit
    )

    if len(chats) == 0:
        await Tags.delete_tag_by_tag_name_and_user_id(form_data.name, user.id)
...
async def get_chat_tags_by_id(id: str, user=Depends(get_verified_user)):
    tags = await Tags.get_tags_by_chat_id_and_user_id(id, user.id)
...
async def add_chat_tag_by_id(
    ...
    if form_data.tag_name not in tags:
        tag = await Tags.add_tag_to_chat(user.id, form_data)
...
async def delete_chat_tag_by_id(
    ...
    result = await Tags.delete_tag_by_tag_name_and_chat_id_and_user_id(
...
async def delete_all_chat_tags_by_id(id: str, user=Depends(get_verified_user)):
    result = await Tags.delete_tags_by_chat_id_and_user_id(id, user.id)
