...
from open_webui.apps.webui.internal.db import Session, engine
...
async def run_migrations():
    ...
    try:
        alembic_cfg = Config(
            os.path.join(os.path.abspath(os.path.dirname(__file__)), "alembic.ini")
        )
        db = engine
        async with db.begin() as con:
            alembic_cfg.attributes["connection"] = con
            con.run_sync(run_upgrade, alembic_cfg)
            return

    except Exception:
        log.exception(f"Error: {traceback.print_exc()}")
...
async def get_filter_function_ids(model):
    ...
    filter_ids = [
        {"filter_id": filter_id, "priority": await get_priority(filter_id)}
        for filter_id in filter_ids
        if filter_id in enabled_filter_ids
    ]

    sorted_filters = sorted(filter_ids, key=lambda x: x["priority"])
    filter_ids = [d["filter_id"] for d in sorted_filters]
    return filter_ids
...
async def chat_completion_tools_handler(
    ...
    tools = await get_tools(webui_app, tool_ids, user, custom_params)
    log.info(f"{tools=}")
...
async def chat_completed(form_data: dict, user=Depends(get_verified_user)):
    ...
    sorted_ids = sorted(filter_ids, key=lambda x: x["priority"])
    filter_ids = [x["filter_id"] for x in sorted_ids]
